...
export async function sendConfirmationEmail({ name, email, confirmationUrl, productName }) {
  try {
    const transporter = nodemailer.createTransport({
      host: process.env.EMAIL_HOST,
      port: parseInt(process.env.EMAIL_PORT),
    ...
    const mailOptions = {
      from: `"Pottery Studio" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: `Confirm Your Pottery Request - ${productName || 'Contact'}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          <style>
            ...
            .header { background: linear-gradient(135deg, #d97706 0%, #ea580c 100%); color: white; padding: 30px; text-align: center; border-radius: 8px; }
            .content { padding: 30px; background: #f9fafb; margin: 20px 0; border-radius: 8px; }
            .button { display: inline-block; background: #d97706; color: white; padding: 15px 40px; text-decoration: none; border-radius: 8px; font-weight: bold; margin: 20px 0; }
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p>Thank you for your interest${productName ? ` in ${productName}` : ''}!</p>
              
              <p>To complete your request, please confirm your email by clicking the button below:</p>
              
           <div style="text-align: center;">
            <a href="${confirmationUrl}" class="button" style="color: white; text-decoration: none;">Confirm Request</a>
          </div>
                        
              <div class="warning">
                <strong>Link expires in 24 hours.</strong> If you did not make this request, ignore this email.
              </div>
